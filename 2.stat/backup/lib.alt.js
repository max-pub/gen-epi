...
export function calc(cgmlstPairs, epiDist, VARS = {}, KEY = 'delay') {
	...
	for (let day of VARS.delayDays) {
		for (let loc of VARS.locationLayers) {
			...
			// let genPairs = cgmlstEpiFilter(cgmlstPairs,epiContacts)
			// console.log(genPairs)
			// return
			...
			for (let deg of VARS.contactDegrees) {
				for (let dist in cgmlstPairs) {
					...
					let stat = { pairs: 0, contacts: 0 }
					for (let [pid1, pid2] of cgmlstContacts) {
						if (!epiContacts[pid1] || !epiContacts[pid2]) {
							// console.log("NO EPI",pid1,pid2)
							continue
						}
						stat.pairs++
						...
						if (!con) console.log("NO CONTACT", pid1)
...
export function cgmlstEpiFilter(cgmlstPairs,epiContacts){
	// console.log(epiContacts)
	let out = {}
	for(let dist in cgmlstPairs){
		for(let [a,b] of cgmlstPairs[dist]){
			// console.log(dist,a,b)
			if(epiContacts[a]?.includes(b)){
				out[dist] ??= []
				out[dist].push([a,b])
			}
		}
